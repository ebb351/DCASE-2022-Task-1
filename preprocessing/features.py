...
class ScatteringExtractor(FeatureExtractor):
    """Scattering Transform feature extractor with log + time-pooling"""
    label = 'scattering'
    description = 'Scattering Transform (Kymatio) + log + mean pooling'

    def __init__(self, fs=44100, J=6, Q=12, shape=32768, log_eps=1e-6, **kwargs):
        kwargs.update({
            'fs': fs,
            'win_length_samples': shape,
            'hop_length_samples': shape,
            'win_length_seconds': shape / fs,
            'hop_length_seconds': shape / fs
        })

        super(ScatteringExtractor, self).__init__(**kwargs)
        self.J = J
        self.Q = Q
        self.shape = shape
        self.log_eps = log_eps

        self.scattering = Scattering1D(J=self.J, shape=self.shape, Q=self.Q, max_order=1)
    ...
    def extract(self, y):
        ...
        if len(y) < self.shape:
            y = np.pad(y, (0, self.shape - len(y)), mode='constant')
        else:
            y = y[:self.shape]

        # Scattering transform
        Sx = self.scattering(y)  # shape: (C, T')

        # Remove zeroth-order (channel 0)
        Sx = Sx[1:, :]  # shape: (C-1, T')

        # log transform
        Sx = np.log(np.abs(Sx) + self.log_eps)

        if Sx.shape[1] % 4 == 0:
            Sx = Sx.reshape(Sx.shape[0], Sx.shape[1] // 4, 4).mean(axis=2)
